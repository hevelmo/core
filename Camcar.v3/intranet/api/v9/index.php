...
function getWSEmpleadosByCumpleanos($fecha) {
    $array = getEmployeesByDataRange('cumpleanos', $fecha, $fecha, 'm', true, 'DESC');
    echo getWSEmpleadosJSON($array);
}
...
function getWSEmpleadosByFechaIngreso($fecha) {
    $array = getEmployeesByDataRange('fecha_ingreso', $fecha, $fecha, 'm', true, 'DESC');
    echo getWSEmpleadosJSON($array);
}
...
function updateFromWS($empleadosDB, $empleadosWS) {
    ...
    foreach($empleadosWS as $keyWS => $empleadoWS) {
        ...
        /*
        $apPat = $empleadoWS['apellido_paterno'];
        $apPat = trim($apPat);
        $apMat = $empleadoWS['apellido_materno'];
        $apMat = trim($apMat);
        $nombres = $empleadoWS['nombres'];
        $nombres = trim($nombres);
        $fullname = $apPat . ' ' . $apMat . ' ' . $nombres;
